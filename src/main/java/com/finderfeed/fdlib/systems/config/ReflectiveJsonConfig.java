 ...
 * 3) If a value of a field is a user-defined class it should implement ReflectiveSerializable interface
 ...
 * config loads from disk or when config is updated by packet.
 */
public abstract class ReflectiveJsonConfig extends JsonConfig {
        ...
        try {
            ...
            if (value instanceof Integer val) {
                ...
                var res = codec.parse(JsonOps.INSTANCE,obj).resultOrPartial(str->{
                    FDLib.LOGGER.error("Error while parsing field: " + name + ", in config: " + this.getName() + ", using default value.");
                    FDLib.LOGGER.error(str);
                });
                if (res.isPresent()){
    ...
    private <T> void hackyCodecUse(T defaultVal,Codec<T> codec,JsonObject object,String name){
        var result = codec.encodeStart(JsonOps.INSTANCE,defaultVal).resultOrPartial(str->{
            FDLib.LOGGER.error("Error while encoding field: " + name + ", in config: " + this.getName() + ", tell mod dev that he is dumb.");
            FDLib.LOGGER.error(str);
        });
        if (result.isPresent()){
